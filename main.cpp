    ...
    while (!quit) {
        while (SDL_PollEvent(&e) != 0) {
            if (e.type == SDL_QUIT) {
                ...
                if (e.button.button == SDL_BUTTON_LEFT && !right && !left) {
                    isAttack = true;
        ...
        for (int i = 0; i < 30; i++) {
            ...
            if (isAttack && direcright && enemy_g[i].enemy_x - p1.player_x < 80 && enemy_g[i].enemy_x - p1.player_x > 0) {
                if (direcright) {
            ...
            else if (isAttack && direcleft && enemy_g[i].enemy_x - p1.player_x > -80 && enemy_g[i].enemy_x - p1.player_x < 0) {
                if (direcleft) {
                    enemy_g[i].goblin_hit = true;
                    enemy_g[i].goblin_hit_start = SDL_GetTicks();
                    if (goblin_hit_yes) {
                ...
                if (enemy_g[i].isattack_goblin) {
                    ...
                    if (abs(enemy_g[i].enemy_x - p1.player_x) < 75 && abs(enemy_g[i].enemy_y - p1.player_y) < 40) {
                        if (!enemy_g[i].prep_attack) {
                            ...
                            if (currentTime1 - enemy_g[i].attack_goblin_start >= 400) {
